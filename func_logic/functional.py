from pycbrf.toolbox import ExchangeRates  # Модуль, которые извлекает обменные курсы ЦБ РФ
...
def get_the_GoogleSheet_table_by_API():
    """
    Функция возвращает переменную service в которой содержится путь к таблице GoogleSheets (получаем доступ к таблице)
    """
    # Подключаемся к учётной записи Google API
    SERVICE_ACCOUNT_FILE = './keys.json'  # путь к вашему json файлу
...
def conversion_dollar_to_ruble():
    """
    Функция отправляет запрос на сервис ЦБ РФ и возвращает курс рубля к доллару.
    """
...
def delete_line_in_SQL(sql_data_cache, dict_google_sheet):
    """
    Функция удаляет строку из БД если такого номера заказа в таблице GoogleSheets не нашлось.
...
def rewrite_data_in_SQL(dict_google_sheet, sql_data_cache):
    """
    Функция перезаписывает данные в БД (если в БД не хватало данных, у данного номера заказа (не успели заполнить)
    ...
    for i in dict_google_sheet:
        if i in sql_data_cache and dict_google_sheet[i] != sql_data_cache[i] and dict_google_sheet[i] != "DEFAULT":
            ...
            try:
                datetime.datetime.strptime(dict_google_sheet[i][2], "%d.%m.%Y")  # Проверка правильности ввода даты
                sql_add_line(dict_google_sheet[i][0], i, dict_google_sheet[i][1], dict_google_sheet[i][2])  # Обновляем строку с номером заказ со всеми данными из словаря
            except ValueError:
                sql_add_line(dict_google_sheet[i][0], i, dict_google_sheet[i][1], '01.01.1970')
