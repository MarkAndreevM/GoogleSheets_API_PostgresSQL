...
def get_sheets_data(service, sheet_id):
    """
    Возвращает данные из таблицы GoogleSheet, перезаписанные в словарь, где ключ это - номер заказа
...
def rework_res(res: list, length=4):
    ...
    for i in res:
        ...
        i[2] = round(float(i[2]) * rub, 2) if i[2].isdigit() else 0
...
if __name__ == "__main__":
    ...
    rub = convert_dollar_in_rub  # Записываем полученное значение в новую ячейку памяти (избегаем timeout)

    service = get_the_GoogleSheet_table_by_API()  # переменная, в которой содержится путь к GoogleSheets таблице

    sql_start()  # Подключаемся к БД, создаем таблицу если таблицы нет
    sql_data_cache = sql_get_select_all_data()
    sql_data_cache = {str(i[1]): (i[0], round(i[2], 2), str(i[3])) for i in sql_data_cache}  # превращаем кэшируемые данные из SQL в словарь, где ключ - это номер заказа
